class VocbenchImporter
  ...
  def initialize(object, default_namespace_url, logger = Rails.logger, publish = true, verbose = false, prefix_for_altlabels = nil)
    @file = case object
              when File
                File.open(object)
              when Array
    ...
    @verbose = verbose
    @logger = logger
    @prefix_for_altlabels = prefix_for_altlabels
...
# VOCBENCH-SPECIFIC: Replace Vobenchurl with namespace + a leading underscore for the id to make it a valid origin
      line.gsub!("http://thesauri.dainst.org/schemes/1/concepts/","http://thesauri.dainst.org/_")
# VOCBENCH-SPECIFIC: Replace hash (#l[somenumber]) in origins with dash
      line.gsub!(/#l(\d)/,'-l\1')
